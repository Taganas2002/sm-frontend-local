    ...
    <Box m="20px">
      ...
      <Box display="flex" flexWrap="wrap" gap={1.2} alignItems="center" mb={1.5}>
        ...
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} label={t.filterByGroup}/>}
          clearOnBlur={false}
        />
      ...
      <Box display="flex" gap={1} flexWrap="wrap" mb={1}>
        <Chip label={`${t.grossLabel}: ${money.format(totals.gross || 0)}`} color="warning" />
        <Chip label={`${t.netLabel}: ${money.format(totals.net || 0)}`} color="success" />
        <Chip label={`${t.paidLabel}: ${money.format(totals.paid || 0)}`} color="info" />
        <Chip label={`${t.unpaidLabel}: ${money.format(totals.unpaid || 0)}`} color="error" />
        <Chip label={`${t.rowsLabel}: ${counts.totalEarnings || 0}`} variant="outlined" />
      </Box>
      ...
      >
        <Box display="flex" gap={1} alignItems="center">
          <Chip label={`${t.selectedLabel}: ${sel.size}`} variant="outlined" />
          <Chip label={`${t.toPayLabel}: ${money.format(netSelected)}`} color="success" />
        </Box>
