...
import { readTeacherImportFile } from "../../utils/teacherImportParse";
import { downloadTeacherImportTemplate } from "../../utils/importTemplateWorkbook";
import { primaryImportBtnSx } from "../../utils/importUi";

const TeacherImportDialog = ({ open, onClose, language, onImported }) => {
  ...
  const translateParseError = (code) => {
    ...
    if (code === "noRows") return t.teacherImportNoRows || "No teacher names were found.";
    return code;
  };
  ...
  const onPickFile = async (e) => {
    ...
    setResult(null);
    try {
      const { rows, errors } = await readTeacherImportFile(file);
  ...
  const runImport = async () => {
    ...
    for (let i = 0; i < toImport.length; i++) {
      ...
      try {
        await createTeacher(row.payload);
        ok += 1;
      } catch (err) {
        const msg =
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      ...
      >
        {t.importTeachers || "Import teachers from Excel"}
      </DialogTitle>
      <DialogContent sx={{ pt: 2 }}>
        <Typography variant="body2" color="text.secondary" paragraph>
          {t.importTeachersHint
            || "Only the teacher name is required. Phone, email, card UID and QR are optional."}
        </Typography>
        <Box display="flex" flexWrap="wrap" gap={1.5} alignItems="center" mb={2}>
          ...
          <Button
            ...
            onClick={() => downloadTeacherImportTemplate(language)}
            sx={{
              borderWidth: 2,
              fontWeight: 600,
          ...
          >
            {t.downloadTeacherTemplate || "Download template"}
          </Button>
          {fileLabel ? (
        ...
        {importing ? (
          <Box sx={{ mb: 2 }}>
            <Typography variant="body2" gutterBottom>
              {t.importingTeachers || "Importing…"} {progress.done}/{progress.total}
            </Typography>
        ...
        {result ? (
          <Alert severity={result.failures.length ? "warning" : "success"} sx={{ mb: 2 }}>
            {(t.importTeachersResult || "{{ok}} created, {{fail}} failed.")
